...
writer = SummaryWriter('board/experiment_cifar10')
...
optimizer = torch.optim.SGD(
    [{'params': model.parameters(), 'initial_lr': 1e-3}],
    lr=learning_rate, weight_decay = 0.001, momentum = 0.9
)
lr_scheduler = CosineAnnealingLR(
    optimizer,eta_min=1e-5,T_max=10000
)
...
for epoch in range(num_epochs):
    ...
    for i, (images, labels) in enumerate(train_loader):
        ...
        writer.add_scalar('Loss/step-loss', loss, step)

    lr_scheduler.step(epoch)
    writer.add_scalar(
        'optimizer/lr', optimizer.param_groups[0]['lr'], epoch)

    print('Epoch [{}/{}], Loss: {:.4f}'
          .format(epoch + 1, num_epochs, epoch_loss))
    writer.add_scalar('Loss/epoch-loss', epoch_loss, epoch + 1)
    # Validation
    with torch.no_grad():
        correct = 0
        total = 0
        ...
        writer.add_scalar('Accuracy/validation', 100 * correct / total, epoch + 1)
